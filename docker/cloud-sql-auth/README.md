...
Or use the `cloud-sql-auth` service in docker-compose.yml (set `CLOUD_SQL_CONNECTION_NAME` in `.env`). The same `docker/.env` is used for the app (DB, overlay API, payment link API key, YouTube OAuth and push config, Stripe webhook, worker RTMP output URL, etc.).
